...
class PCB():
    ...
    def __init__(self, pid, db, instructions, path, pri):
        self._pid = pid
        self._baseDir = db
        self._pc = 0
...
class PcbTable():
    ...
    # ----------------------- Setters ------------------------------------------------
    def crearPcb(self, program, db, pri):
        self.increasePID()
        pcb = PCB(self._contadorPid, db, len(program._instructions), program.name,pri)
        self.addPcb(pcb)
        return pcb
...
class NewInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        pcb = self.table().crearPcb(program, self.loader().getDirBase(), pri)
