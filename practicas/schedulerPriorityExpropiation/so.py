...
class AbstractInterruptionHandler():
    ...
    def expropiate(self, pcb):
        pcbInCPU = self.table().returnRunning()
        self.dispatcher().save(pcbInCPU)
        self.planificador().addNewPCB(pcbInCPU)
        self.dispatcher().load(pcb)
        self.table().setRunning(pcb)
...
class KillInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        if (self.planificador().noIsEmpty()):
            pcbNext = self.planificador().returnNext()
            self.table().setRunning(pcbNext)
            log.logger.info("pcbNext" + str(pcbNext))
            self.dispatcher().load(pcbNext)
        else:
            HARDWARE.cpu.pc = -1
...
class IoInInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        if self.planificador().noIsEmpty():
            ...
            self.dispatcher().load(pcb)
            log.logger.info("ReadyQueue" + str(self.planificador().readyQueue()))
            log.logger.info("Running Now" + str(self.table().returnRunning()))
        else:
            HARDWARE.cpu.pc = -1
...
class IoOutInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        pcb.setState("Ready")
        log.logger.info("pcb que retorna" + str(pcb))
...
class Kernel():
    ...
    def __init__(self):
        ...
        self._loader = Loader()
        self._dispatcher = Dispatcher()
        self._planificador = Planificador(self.dispatcher(), 5)
    ...
    def runner(self, pcb):
        log.logger.info("\n Executing program: {name}".format(name=pcb.getPath()))
        log.logger.info(HARDWARE)

        HARDWARE.cpu.pc = pcb.getBaseDir()
